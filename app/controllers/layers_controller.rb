...
layout 'layerdetail', :only => [:show,  :export, :metadata]
before_filter :login_or_oauth_required , :except => [:wms, :wms2, :show_kml, :show, :index, :metadata, :maps, :thumb, :geosearch, :tile]  
...
def thumb
  m_nypl_id =  @layer.maps.first ? @layer.maps.first.nypl_digital_id : 0
  thumb = "http://images.nypl.org/?t=t&id=#{m_nypl_id}" 
  redirect_to thumb
end
...
def show
  @current_tab = "show"
  @selected_tab = 0
  #@disabled_tabs = ["digitize","metadata"]
...
def update_year
  ...
  spawn do
    @layer.update_digitizer_features
  end
...
def publish
  if @layer.rectified_percent < 100 
   ...
   def wms()
      ...
      map = MapObj.new(File.join(RAILS_ROOT, '/db/mapfiles/wms.map'))
      ...
      "http://" + request.host_with_port + ActionController::Base.relative_url_root + "/layers/wms/#{@layer.id}")
   ...
   def wms2
     ...
     ows = OWSRequest.new
      ...
      # logger.info map.getProjection
      map.setMetaData("wms_onlineresource",
      "http://" + request.host_with_port + ActionController::Base.relative_url_root + "/layers/wms2")
      unless @layer_name
